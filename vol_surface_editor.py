...
try{{
const key = 'v3d_' + D.ccy;
const saved = window.parent.localStorage.getItem(key);
if(saved){{
  const s = JSON.parse(saved);
  if(Date.now() - s.ts < 60000){{
    D.values = s.vals;
    D.savedMode = s.mode;
    // Recalculate zMin/zMax from loaded values
    let mn=Infinity,mx=-Infinity;
    for(let r of D.values)for(let v of r){{if(v<mn)mn=v;if(v>mx)mx=v;}}
    const rng=mx-mn;
    D.zMin=Math.floor((mn-rng*0.05)/10)*10;
    D.zMax=Math.ceil((mx+rng*0.05)/10)*10;
...
def render_vol_surface_editor(ccy: str, atm_surface: pd.DataFrame) -> pd.DataFrame:
    # Check for saved edits in localStorage BEFORE init
    force_update = False
    
    if HAS_JS_EVAL:
        try:
            key = f'v3d_{ccy}'
            saved = streamlit_js_eval(js_code=f"localStorage.getItem('{key}')", key=f"ls_{ccy}_{id(atm_surface)}")
            if saved and saved != 'null':
                data = json.loads(saved)
                if 'ts' in data and 'vals' in data:
                    updated = data['vals']
                    mode = data.get('mode', 'vol')
                    ...
                    streamlit_js_eval(js_code=f"localStorage.removeItem('{key}')", key=f"lsc_{ccy}")
    ...
    st.markdown("""<style>div.stButton>button{font-weight:600!important}div.stButton>button[kind="primary"]{background:linear-gradient(135deg,#22c55e,#16a34a)!important}</style>""", unsafe_allow_html=True)
    ...
    st.caption("Drag points • Green=up, Red=down • Neighbors adjust with smoothing")
