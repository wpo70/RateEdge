...
def _init_state(ccy: str, surface: pd.DataFrame) -> None:
    ...
    for k in ["working", "base", "history", "redo_stack", "view_mode", "smoothing"]:
        if k not in ed:
            ed[k] = {}
...
#btn{{position:absolute;bottom:10px;right:10px;background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;border:none;padding:10px 24px;border-radius:6px;font-weight:600;cursor:pointer;display:none;font-size:13px}}
#btn.show{{display:block}}
#lockBtn{{position:absolute;bottom:10px;right:140px;background:#334155;color:#fff;border:none;padding:10px 16px;border-radius:6px;font-weight:500;cursor:pointer;font-size:12px}}
...
// Always use prompt - clipboard often blocked in iframes
const copied = prompt('Copy this data (Ctrl+A, Ctrl+C), then paste below:', payload);
if(copied !== null){{
  btn.textContent='Now paste below & click CONFIRM';
  btn.style.background='#22c55e';
  st.textContent='Copy the data from popup, paste below, click CONFIRM';
...
div[data-testid="stRadio"] label{color:#ffffff!important}
    ...
    st.caption("Drag points • Green=up, Red=down • Click Apply then paste below")
    _render_3d_editor(working, ccy, view_mode, smoothing, base)
    
    # Confirm section - VISIBLE, not in expander
    st.markdown("---")
    st.markdown("#### 📋 Step 2: Confirm Changes")
    st.caption("After clicking Apply above, paste the copied data here and click Confirm")
    col_input, col_btn = st.columns([3, 1])
    with col_input:
        confirm_data = st.text_input("Paste data:", key=f"confirm_{ccy}", label_visibility="collapsed", placeholder="Paste the Apply data here...")
    with col_btn:
        confirm_clicked = st.button("✅ CONFIRM", key=f"confirm_btn_{ccy}", type="primary", use_container_width=True)
    
    if confirm_clicked:
        if confirm_data:
            try:
                import base64
                payload = json.loads(base64.b64decode(confirm_data).decode())
                if payload.get('ccy') == ccy:
                    ...
                    st.success("✅ Changes applied to working surface!")
                    st.rerun()
                else:
                    st.error(f"Data is for {payload.get('ccy')}, not {ccy}")
            except Exception as e:
                st.error(f"Invalid data: {e}")
        else:
            st.warning("Paste the Apply data first")
