...
_LOG_PATH = _os_log.path.join(_os_log.path.dirname(_os_log.path.abspath(__file__)), "sdr_fetcher.log")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s  %(levelname)-8s  %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.StreamHandler(),                 # console (when run in a window)
        logging.FileHandler(_LOG_PATH, encoding="utf-8"),  # file (for headless task)
    ],
)
...
def map_row(row: dict, trade_date: date):
    ...
    # STRICT option classification: a genuine IR option (swaption / cap / floor /
    # straddle) carries the "/O " marker in its ANNA-DSB UPI FISN. Embedded-option
    # codes (OPET/MDET/CANC/...) on a plain swap FISN are cancellable / early-
    # termination SWAPS — not option prints — and previously leaked into the
    # options tape as EC / MDET / XCS / OTH rows.
    is_option = "/O " in fisn
    ...
    # Junk gate for OPTION rows: a genuine option print carries a strike OR a
    # premium. Rows with neither are mis-decoded products (EC / XCS / OTH etc.)
    # and must not enter the table. CFS (forward-start IRS) rows are exempt —
    # they are a deliberate separate product class with no strike/premium.
    if trade_type == "OPT":
        _sr, _sp = strike_raw, strike_pct
        _pa = parse_num(row.get("optionPremiumAmount"))
        if _sp is None and (_pa is None or _pa <= 0):
            return None
...
def fetch_dtcc(dt_low: datetime, dt_high: datetime,
    ...
    try:
        ...
        _code = getattr(e.response, "status_code", "?")
        log.error(f"DTCC API error: {e}")
        return [], f"DTCC_ERROR_{_code}"
    except Exception as e:
        log.error(f"DTCC API error: {e}")
        return [], "DTCC_ERROR"
...
def poll_once(conn, minutes_back: int = 10):
    ...
    raw, _dtcc_status = fetch_dtcc(dt_low, dt_high)
    if not raw:
        return 0, 0, _dtcc_status
    ...
    if not opts:
        return 0, 0, _dtcc_status
    _ins, _upd = upsert_rows(conn, opts)
    return _ins, _upd, _dtcc_status
...
def backfill_day(conn, d: date):
    ...
    raw, _ = fetch_dtcc(dt_low, dt_high)
    if not raw:
...
def main():
    ...
    if args.date_from and args.date_to:
        ...
        while True:
            ...
            # Heartbeat every cycle. Status carries DTCC health so the app can show
            # "fetcher running" (heartbeat fresh) separately from "DTCC API down".
            # If the DB connection has dropped, reconnect so 24/7 running survives
            # transient network/DB blips instead of dying silently.
            try:
                upsert_heartbeat(conn, _cycle_status)
            except Exception as e:
                log.error(f"Heartbeat write failed ({e}); reconnecting DB…")
                try:
                    conn.close()
                except Exception:
                    pass
                try:
                    ...
                    log.error(f"DB reconnect failed: {e2}")
            time.sleep(args.loop * 60)

    else:
        ensure_heartbeat_table(conn)
        ins, upd, _st = poll_once(conn, minutes_back=args.minutes)
        upsert_heartbeat(conn, _st)
        log.info(f"Done: {ins} new, {upd} updated. (DTCC: {_st})")
